#--------------------------------------------------------------------------------
# aliases
export INVOICE_REPOS=~/git/Invoice-Keeper 
alias c='cd ${INVOICE_REPOS}'
alias s='cd ${INVOICE_REPOS}'
...
{
  cp -Rf ${INVOICE_REPOS}/WebContent/WEB-INF/resources/javascripts/  ${TOMCAT_HOME}/webapps/vlok/WEB-INF/resources/
}


function start_hsqldb()
{
  java -cp ~/.m2/repository/org/hsqldb/hsqldb/2.2.8/hsqldb-2.2.8.jar org.hsqldb.Server -database.0 file:${INVOICE_REPOS}/tmpdb/invoice -dbname.0 xdb &
}
...
function tomcat_deploy {
  cp -R  ${INVOICE_REPOS}/target/invoice-keeper.war $TOMCAT_HOME/webapps/
}
